...
namespace TOPRO.HSL.Serial
{
    ...
    public class SerialDeviceBase<TTransform> : SerialBase, IReadWriteNet where TTransform : IByteTransform, new()
    {
        ...
        /// <returns>带成功标志的结果数据对象</returns>
        public OperateResult<string> ReadString(string address, ushort length,Encoding encode)
        {
            return ByteTransformHelper.GetStringResultFromBytes(Read(address, length), byteTransform,encode);
        }
        ...
        public virtual OperateResult Write(string address, string value, Encoding encode) 
        {
            byte[] temp = ByteTransform.TransByte(value, encode);
            if (WordLength == 1) temp = SoftBasic.ArrayExpandToLengthEven(temp);
            return Write(address, temp);
        }
        ...
        /// <param name="encode"></param>
        /// <returns>带有成功标识的结果类对象</returns>
        public virtual OperateResult Write(string address, string value, int length, Encoding encode) 
        {
            byte[] temp = ByteTransform.TransByte(value, encode);
            if (WordLength == 1) temp = SoftBasic.ArrayExpandToLengthEven(temp);
            temp = SoftBasic.ArrayExpandToLength(temp, length);
