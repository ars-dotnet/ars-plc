...
namespace TOPRO.HSL.Core
{
    ...
    public static class ByteTransformHelper
    {
        ...
        /// <returns>转化后的类型</returns>
        public static OperateResult<string> GetStringResultFromBytes(OperateResult<byte[]> result, IByteTransform byteTransform, Encoding encode)
        {
            return GetResultFromBytes(result, m => byteTransform.TransString(m, 0, m.Length, encode));
        }
    }
}
